...
class Wishlist extends AppModel
{
    ...
    static function get_wishlist_ids()
    {
    ...
    {
        $wishlist = \Wfm\App::$app->getProperty('wishlist');
        if($wishlist){
            $ids = implode(',', $wishlist);
            return R::getAll("SELECT * FROM product JOIN product_description ON
        product.id = product_description.product_id WHERE status = 1 AND id IN ($ids) AND language_id = ?", [$lang['id']]);
        }
        return [];
